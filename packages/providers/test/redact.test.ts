...
describe("secret redaction in provider errors", () => {
  ...
  it("redacts Google, OpenAI, OpenRouter, and Groq key patterns", () => {
    expect(redactSecrets("key AIzaSyEXAMPLE1234567890abcdefghij here")).toContain("***");
    expect(redactSecrets("key AIzaSyEXAMPLE1234567890abcdefghij here")).not.toMatch(/AIza\w/);
  ...
  it("a 403 error body echoing the api key is redacted before it reaches the ProviderError", async () => {
    const key = "AIzaSyEXAMPLE1234567890abcdefghij";

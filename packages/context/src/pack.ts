...
export function calculateContextBudget(options: BuildContextPackOptions): ContextBudget {
  ...
  const safetyMargin = options.safetyMarginTokens !== undefined
    ? Math.max(0, options.safetyMarginTokens)
    : Math.max(512, Math.ceil(options.contextWindow * 0.05));
...
export function boundedSlice(content: string, symbol?: RepositorySymbol): { content: string; startLine: number; endLine: number } {
...
export function fitToTokens(content: string, maximum: number): string {

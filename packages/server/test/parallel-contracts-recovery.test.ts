...
describe("CF-08 public contracts and restart safety", () => {
  ...
  it("persists stable dispatch identities and fails closed after a restart instead of dispatching a second Coder", async () => {
    ...
    expect((await createParallelAutonomousRunOrchestrator({ persistence, workspaceService: {} as never, agentRuntime: {} as never }).getRun("parallel-restart"))?.dispatches).toHaveLength(2);

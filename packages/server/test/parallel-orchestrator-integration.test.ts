...
describe("CF-08 real concurrent worktree orchestration", () => {
  ...
  it("proves two Coders overlap in separate real worktrees and safely promotes their clean synthesis", async () => {
    const latch = new Latch(); const catalog = new InMemoryProviderCatalog(); catalog.register(new ParallelProvider(latch)); const firewall = new ForgeZero(); firewall.register(createGenericFreeRecord({ providerId: "parallel-test", modelId: "free-model-1" }));
  ...
  it("fails closed on a real textual Git conflict while retaining every workstream branch", async () => {
    ...
    const catalog = new InMemoryProviderCatalog(); catalog.register(new ParallelProvider(undefined, "textual")); const firewall = new ForgeZero(); firewall.register(createGenericFreeRecord({ providerId: "parallel-test", modelId: "free-model-1" })); const workspaceService = createWorkspaceService({ persistence, worktreeParentDir: worktreeDir }); const runtime = createAgentRuntime({ sessionId: "conflict", eventStore: new EventStore(), persistence, firewall, providerCatalog: catalog, workspacePath: repoDir }); const result = await createParallelAutonomousRunOrchestrator({ workspaceService, agentRuntime: runtime, persistence }).startRun({ sessionId: "conflict", workspacePath: repoDir, goal: "conflict" });
  ...
  it("blocks promotion when a clean Git synthesis fails real global verification", async () => {
    ...
    const catalog = new InMemoryProviderCatalog(); catalog.register(new ParallelProvider(undefined, "semantic")); const firewall = new ForgeZero(); firewall.register(createGenericFreeRecord({ providerId: "parallel-test", modelId: "free-model-1" })); const workspaceService = createWorkspaceService({ persistence, worktreeParentDir: worktreeDir }); const runtime = createAgentRuntime({ sessionId: "semantic", eventStore: new EventStore(), persistence, firewall, providerCatalog: catalog, workspacePath: repoDir }); const result = await createParallelAutonomousRunOrchestrator({ workspaceService, agentRuntime: runtime, persistence }).startRun({ sessionId: "semantic", workspacePath: repoDir, goal: "semantic" });
  ...
  it("uses IntegrationService divergence protection when the user advances the target before promotion", async () => {
    const catalog = new InMemoryProviderCatalog(); catalog.register(new ParallelProvider()); const firewall = new ForgeZero(); firewall.register(createGenericFreeRecord({ providerId: "parallel-test", modelId: "free-model-1" })); const workspaceService = createWorkspaceService({ persistence, worktreeParentDir: worktreeDir }); const runtime = createAgentRuntime({ sessionId: "divergence", eventStore: new EventStore(), persistence, firewall, providerCatalog: catalog, workspacePath: repoDir }); const integration = createIntegrationService({ workspaceService });

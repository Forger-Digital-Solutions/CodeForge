...
export interface ParallelOrchestratorOptions { workspaceService: WorkspaceService; agentRuntime: AgentRuntime; integrationService?: IntegrationService; persistence?: SessionPersistence; onEvent?: (event: ParallelEvent) => void; }
...
export class ParallelAutonomousRunOrchestrator {
  ...
  getRun(runId: string): DurableParallelRun | undefined { return this.store.get(runId); }
  listRuns(sessionId?: string): DurableParallelRun[] { return this.store.list(sessionId); }
  ...
  recoverRun(runId: string): DurableParallelRun | undefined {
    ...
    if (unsafe.length) {
      ...
      run.status = "blocked"; run.error = "PARALLEL_RECOVERY_REVALIDATION_REQUIRED"; this.save(run); this.emit(run, "parallel.recovery.revalidation_required", { workstreamIds: unsafe.map((dispatch) => dispatch.workstreamId) });
    }
    return run;
  }
  private async git(cwd: string, args: string[]) { return execFile("git", args, { cwd, env: { ...getSanitizedEnvForChild(), GIT_TERMINAL_PROMPT: "0" } }); }
  private save(run: DurableParallelRun): void { run.updatedAt = new Date().toISOString(); this.store.save(run); }
  private emit(run: DurableParallelRun, type: string, payload: Record<string, unknown> = {}, workstreamId?: string): void { this.store.emit(run, type, payload, workstreamId); }
  ...
  private async finalizeCancellation(run: DurableParallelRun): Promise<void> {
    ...
    this.save(run);
    this.emit(run, "parallel.cancellation.finalized", { retainedDirty: run.dispatches.filter((dispatch) => dispatch.cleanup === "retained_dirty").map((dispatch) => dispatch.workstreamId), branchesPreserved: run.dispatches.filter((dispatch) => dispatch.branchPreserved).map((dispatch) => dispatch.workstreamId) });
  }
  ...
  private async synthesizeAndPromote(params: { run: DurableParallelRun; target: ForgeWorkspace; signal: AbortSignal; verificationCommands: string[]; privateAgentContext?: PrivateAgentContext }): Promise<ParallelRunResult> {
    ...
    if (!synthesisWorkspace) { run.status = "blocked"; run.error = "PARALLEL_RECOVERY_REVALIDATION_REQUIRED"; this.save(run); this.emit(run, "parallel.recovery.revalidation_required", { reason: "SYNTHESIS_WORKSPACE_UNKNOWN" }); return this.result(run, []); }
    ...
    for (const id of synthesis.order) {
      ...
      if (!stream?.resultRevision) { run.status = "blocked"; run.error = "PARALLEL_SYNTHESIS_SOURCE_MISSING"; this.save(run); this.emit(run, "synthesis.blocked", { reason: "PARALLEL_SYNTHESIS_SOURCE_MISSING" }, id); return this.result(run, []); }
      if (stream.resultRevision !== run.baseRevision) {
        ...
        if (conflicted) {
          ...
          run.status = "blocked"; run.error = "SYNTHESIS_CONFLICT_UNRESOLVED"; this.save(run); this.emit(run, "synthesis.conflict", { paths }, id); return this.result(run, []);
        }
      }
      ...
      this.save(run); this.emit(run, "synthesis.workstream_added", { sourceRevision: stream.resultRevision, resultingRevision }, id);
    }

    run.status = "verifying"; this.save(run); this.emit(run, "global_verification.started");
    const globalVerification = await this.verify(synthesisPath, params.verificationCommands, signal, run);
    this.emit(run, "global_verification.completed", { passed: !globalVerification.some((result) => result.failed > 0) });
    if (globalVerification.some((result) => result.failed > 0)) { run.status = "blocked"; run.error = "GLOBAL_VERIFICATION_FAILED"; this.save(run); return this.result(run, globalVerification); }

    run.status = "reviewing"; this.save(run); this.emit(run, "global_review.started");
    ...
    this.emit(run, "global_review.completed", { passed: globalReview.status === "completed" && review?.verdict === "pass" });
    if (globalReview.status !== "completed" || review?.verdict !== "pass") { run.status = signal.aborted ? "cancelled" : "blocked"; run.error = signal.aborted ? "PARALLEL_RUN_CANCELLED" : "GLOBAL_REVIEW_BLOCKED"; this.save(run); return this.result(run, globalVerification); }

    run.status = "integrating"; this.save(run); this.emit(run, "promotion.started");
    ...
    this.save(run); this.emit(run, promotion.status === "integrated" ? "promotion.completed" : "promotion.blocked", { code: run.error });
  ...
  async resumeRun(runId: string, options: ParallelResumeOptions = {}): Promise<ParallelRunResult> {
    ...
    if (!unsafe.length && run.status === "executing" && !run.synthesis && run.plan) {
      ...
      if (complete && target && head === run.baseRevision) {
        ...
        run.status = "synthesizing"; this.save(run); this.emit(run, "synthesis.started", { recovered: true });
      }
    }
    if (unsafe.length || run.status !== "synthesizing" || !run.synthesis) {
      run.dispatches = run.dispatches.map((dispatch) => unsafe.some((candidate) => candidate.dispatchId === dispatch.dispatchId) ? { ...dispatch, state: "revalidation_required" } : dispatch);
      run.status = "blocked"; run.error = "PARALLEL_RECOVERY_REVALIDATION_REQUIRED"; this.save(run);
      this.emit(run, "parallel.recovery.revalidation_required", { reason: unsafe.length ? "UNSAFE_DISPATCHES" : "PHASE_NOT_RESUMABLE", workstreamIds: unsafe.map((dispatch) => dispatch.workstreamId) });
      return this.result(run, []);
    }
    const revalidation = await this.revalidateSynthesis(run);
    if (!revalidation.ok) {
      run.status = "blocked"; run.error = "PARALLEL_RECOVERY_REVALIDATION_REQUIRED"; this.save(run);
      this.emit(run, "parallel.recovery.revalidation_required", { reason: revalidation.reason });
      return this.result(run, []);
    }
    const target = this.options.workspaceService.getWorkspace(run.workspaceId);
    if (!target) { run.status = "blocked"; run.error = "PARALLEL_RECOVERY_REVALIDATION_REQUIRED"; this.save(run); this.emit(run, "parallel.recovery.revalidation_required", { reason: "TARGET_WORKSPACE_UNKNOWN" }); return this.result(run, []); }
    this.emit(run, "parallel.recovery.resumed", { synthesisHead: revalidation.head, included: run.synthesis.included.map((included) => included.workstreamId), pending: run.synthesis.order.filter((id) => !run.synthesis!.included.some((included) => included.workstreamId === id)) });
    ...
    try {
      ...
      this.save(run); this.emit(run, run.status === "cancelled" ? "parallel.run.cancelled" : "parallel.run.failed", { error: run.error });
      return this.result(run, []);
    } finally {
  ...
  async startRun(input: ParallelRunOptions): Promise<ParallelRunResult> {
    ...
    try {
      ...
      if (this.options.persistence && !this.options.persistence.getSession(input.sessionId)) this.options.persistence.upsertSession({ id: input.sessionId, title: input.goal.slice(0, 80), createdAt: now, updatedAt: now, status: "running" });
      run = { kind: "parallel_run", id: runId, sessionId: input.sessionId, workspaceId: target.id, goal: input.goal, status: "planning", baseRevision, workstreams: [], dispatches: [], contracts: [], usage: emptyParallelRunUsage(), createdAt: now, updatedAt: now }; this.save(run); this.emit(run, "parallel.plan.created");
      ...
      if (planner.status !== "completed" || !planner.structuredData) { run.status = "blocked"; run.error = planner.error ?? "PARALLEL_PLANNER_FAILED"; this.save(run); return this.result(run, []); }
      ...
      if (!validation.valid) { run.status = "blocked"; run.error = `PARALLEL_PLAN_INVALID: ${validation.error}`; this.save(run); return this.result(run, []); }
      run.verificationCommands = input.verificationCommands ?? plan.globalVerificationCommands ?? [];
      this.emit(run, "parallel.plan.validated", { order: validation.orderedWorkstreamIds }); run.status = "executing"; this.save(run);
      ...
      const execute = async (workstream: EngineeringWorkstream): Promise<WorkstreamResult> => {
        ...
        try {
          ...
          run!.dispatches = [...run!.dispatches.filter((dispatch) => dispatch.workstreamId !== workstream.id), { workstreamId: workstream.id, dispatchId: `${runId}:${workstream.id}`, workspaceId: workspace.id, worktreeId: workspace.id, branch: workspace.branch, state: "dispatched" }]; this.save(run!);
          lease = this.options.workspaceService.acquireLease(workspace.id, `${runId}:${workstream.id}`, "write"); this.emit(run!, "workstream.dispatched", { workspaceId: workspace.id, branch: workspace.branch }, workstream.id); this.emit(run!, "workstream.started", { workspaceId: workspace.id }, workstream.id);
          run!.dispatches = run!.dispatches.map((dispatch) => dispatch.workstreamId === workstream.id ? { ...dispatch, state: "active" } : dispatch); this.save(run!);
          ...
          for (;;) {
            coder = await this.options.agentRuntime.executeAgentRun({ runId: `${runId}:${workstream.id}`, agentId: "coder", role: "coder", goal: workstream.objective, taskPlan: JSON.stringify({ workstream: workstream.id, contractsConsumed: contracts }), workspaceId: workspace.id, workspacePath: workspace.rootPath, permissions: { read: true, search: true, write: true, executeCommand: true, network: false }, ...(coderPrivateContext ? { initialContext: coderPrivateContext } : {}), ...(reviewFeedback ? { reviewFeedback } : {}), signal });
            if (signal.aborted || coder.status === "cancelled") throw new Error("PARALLEL_RUN_CANCELLED");
            this.emit(run!, "workstream.reviewing", { revisionRound }, workstream.id);
            ...
            if (coder.status !== "completed" || !reviewData || reviewData.verdict === "pass" || revisionRound >= maxRevisionRounds) break;
            revisionRound++;
            reviewFeedback = JSON.stringify({ verdict: "revision_required", findings: publicFindings(reviewData.findings) });
            this.emit(run!, "workstream.revising", { revisionRound, findingIds: reviewData.findings.map((finding) => finding.id) }, workstream.id);
          }
          ...
          if (!failed) for (const contractId of workstream.contractsProduced ?? []) { const contract = publishedContract(contractId, workstream.id, consumersFor(contractId), revision, result.evidence); result.contractsProduced.push(contract); registry.publish(contract); this.emit(run!, "contract.published", { contractId, revision }, workstream.id); }
          run!.workstreams = [...run!.workstreams.filter((item) => item.workstreamId !== workstream.id), result]; run!.dispatches = run!.dispatches.map((dispatch) => dispatch.workstreamId === workstream.id ? { ...dispatch, state: "completed" } : dispatch); run!.contracts = registry.all(); this.save(run!); this.emit(run!, failed ? "workstream.blocked" : "workstream.completed", { resultRevision: revision }, workstream.id); return result;
        } catch (error) {
      ...
      run.workstreams = [...results.values()]; run.contracts = registry.all(); this.save(run);
      if (run.workstreams.length !== plan.workstreams.length || run.workstreams.some((result) => result.status !== "completed")) { run.status = signal.aborted ? "cancelled" : "blocked"; run.error = signal.aborted ? "PARALLEL_RUN_CANCELLED" : "PARALLEL_WORKSTREAM_BLOCKED"; this.save(run); return this.result(run, []); }
      run.status = "synthesizing"; this.save(run); this.emit(run, "synthesis.started");
      ...
      run.synthesis = { workspaceId: synthesisWorkspace.id, worktreeId: synthesisWorkspace.id, branch: synthesisWorkspace.branch, order: validation.orderedWorkstreamIds!, included: [], conflicts: [] }; this.save(run);
      ...
      if (!run) throw error;
      run.status = signal.aborted ? "cancelled" : "failed"; run.error = signal.aborted ? "PARALLEL_RUN_CANCELLED" : error instanceof Error ? error.message : String(error); this.save(run); this.emit(run, run.status === "cancelled" ? "parallel.run.cancelled" : "parallel.run.failed", { error: run.error }); return this.result(run, []);
    } finally {
      if (run?.status === "cancelled") await this.finalizeCancellation(run);

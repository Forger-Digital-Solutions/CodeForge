...
export class RemotePublicationService {
  ...
  private async git(cwd: string, args: string[], signal?: AbortSignal): Promise<string> { return (await execFile("git", args, { cwd, signal, windowsHide: true, env: { ...getSanitizedEnvForChild(), GIT_TERMINAL_PROMPT: "0" } })).stdout; }

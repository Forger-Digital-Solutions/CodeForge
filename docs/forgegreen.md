...
`EfficiencyReceipt` records bounded, inspectable work quantities: context requested/delivered tokens, tokens avoided, cache hits, duplicate requests avoided, fallback use, reason codes, repository generation, and policy version. It does not expose a universal green score. FG-8's `SustainabilityReceipt` (below) is a separate, explicitly-labeled record for energy/carbon and resource accounting — it never merges into or reinterprets `EfficiencyReceipt`.

## FG-8 — sustainability & resource measurement

FG-8 is ForgeGreen's measurement layer: it answers, with evidence rather than marketing estimates, how many requests/tokens/tool calls/verification commands a run used, what was avoided and why, and — only when defensible — what that cost in energy and carbon. It is observational, exactly like FG-1E's ledger and FG-7's coverage authority: it never returns a permission, approval, verification, routing, or completion decision, and it never overrides `RouteReceipt`/`FinancialReceipt` classification. Full contract: `docs/codeforge-forgegreen-measurement-contract.md`.

**Versioned normalization boundary.** `createSustainabilityReceipt` never reads `AgentUsage`, `ToolExecutionRecord`, `ForgeGreenLedgerTotals`, `RouteReceipt`, or `FinancialReceipt` directly. `normalizeMeasurementInput` (`measurement-normalization.ts`) first maps them into an FG-8-owned, versioned `NormalizedMeasurementInput`, structurally and defensively — a missing or renamed upstream field becomes `undefined`/`"unavailable"`, never a thrown error or a fabricated `0`. Every receipt persists the normalization version it was built with, so historical receipts stay interpretable even as upstream types evolve.

**Coverage, not just confidence.** Every accounting section (tokens, tools, verification, routing, context, timing) carries a `MetricCoverageClass` — `directly_measured`, `provider_reported`, `derived_from_authoritative_telemetry`, `replayed`, `simulated`, `estimated`, or `unavailable` — describing where the number came from. `SustainabilityConfidence` (`DIRECT` … `INSUFFICIENT_DATA`) separately grades how much to trust an *estimate* once it is one. The two are never conflated: energy/carbon fields carry their own `sourceType`/`confidence` and never inherit a token field's coverage class.

**Waste taxonomy.** `classifyWaste` is conservative by construction: an entry is emitted only when its source quantity is actually present. `prevented_waste` entries are drawn only from counters that are themselves mechanism-proven counterfactuals already tracked by earlier FG phases (FG-1C duplicate suppression, FG-5/FG-6 verification reuse/avoidance, FG-3D context-page reuse) — never from a speculative baseline. `verification_overhead` is tracked, never counted as waste; `context_waste` is deliberately never emitted in this phase — no conservative, defensible per-run heuristic for "unnecessary context" exists yet.

**Baseline / counterfactual framework.** Four deterministic, zero-network comparisons (`baseline.ts`): Baseline A (fixed-model, no smart routing), Baseline B (naive full context — requires an explicit `ContextComparisonPopulation` stating exactly what "full context" means for that receipt; repository size is never equated with eligible model context, and a missing population means Baseline B is simply absent, never guessed), Baseline C (naive sequential retry), and Baseline D (verification overhead, reported for transparency only — its numerator/denominator are always `undefined`, so verification can never be represented as a savings source). Every comparison persists its kind, policy version, comparison basis (`measured`/`replayed`/`simulated`/`estimated`), an input-evidence hash, assumptions, numerator, denominator, units, and confidence — never only a percentage. `derivedSavingsPercent` computes a percentage on demand and only when both numerator and denominator are defensible and non-zero.

**Energy/carbon estimation.** Pluggable and versioned (`energy-estimator.ts`): no single hard-coded watts-per-token constant. `InsufficientDataEstimator` is the production default and always reports `INSUFFICIENT_DATA` with `undefined` numbers — missing data stays missing, never `0`. `ReferenceHeuristicEstimator` is a deterministic, openly-labeled non-authoritative fixture used only to prove the interface is pluggable and testable. `hardware-telemetry.ts` defines the adapter interface for future CPU/GPU/power telemetry (NVIDIA/AMD/generic) plus a `NullHardwareTelemetryAdapter`; this phase ships no live polling and does not depend on the developer's current hardware.

**Cross-receipt integrity.** When a `SustainabilityReceipt` references a `RouteReceipt`/`FinancialReceipt`, `checkCrossReceiptIntegrity` verifies task/provider/model consistency and the financial free/paid classification — it never independently reclassifies either authority. A mismatch is surfaced as `measurementStatus: "incomplete"` with the mismatch reason codes, never silently reconciled. **FG-8R** found `RouteReceipt`/`FinancialReceipt` are unused in production (`createRouteReceipt`/`createFinancialReceipt` are called nowhere outside their own definition file) and wired the live path instead: the `FreeModelRecord` ForgeZero verified for the model `AgentUsage` shows actually executed, cross-checked against any persisted 8-Bit `DecisionReceipt`s for the run. Baseline B (above) is likewise now live-wired via a bounded, zero-duplication read-model over `RepositoryIntelligence` (`live-context-adapter.ts`) instead of only synthetic fixtures.

**Non-fatal, observable failure.** A measurement failure (impossible input, cross-receipt mismatch, an unexpected error) never affects the underlying agent run: `persistForgeGreenSustainabilityReceipt` in `AgentRuntime` catches every failure, emits a best-effort `forgegreen.measurement_failed` event, and persists an explicit `measurementStatus: "failed"` receipt via `createFailedSustainabilityReceipt` — never a zero-value receipt standing in for the failure.

**Durability.** `SustainabilityReceiptStore` persists through the same `ISessionPersistence` abstraction as the FG-1E ledger and 8-Bit's receipts (`forgegreen_sustainability_receipt` work-item kind, append-only via `insertIfAbsent`) — no second database. Finalization (`finalizeSustainabilityReceipt`) makes a receipt immutable; finalizing twice is rejected.

**Events.** `forgegreen.run_started`, `model_usage_recorded`, `tool_usage_recorded`, `verification_usage_recorded`, `baseline_generated`, `energy_estimated`, `run_finalized`, and `measurement_failed` are emitted once per run (aggregated), never per token/tool-call — counts/ids/classifications only, never prompt or source content.
...
## FG-9 — optimization & efficiency policy

FG-9 (Phase 4) is ForgeGreen's first phase with any ability to ACT, not just measure — and only inside four explicitly named, narrowly scoped resource-control surfaces. Full contract: `docs/codeforge-forgegreen-optimization-policy.md`.

**Policy modes.** `OFF` / `SHADOW` (candidates identified and recorded, execution never altered) / `ACTIVE_SAFE` (a graduated kind may actually take effect), resolved per optimization kind via a versioned graduation registry — never one global switch. `createOptimizationDecision` enforces at construction that a non-`ACTIVE_SAFE` kind can never carry status `APPLIED`.

**Four candidate classes, one graduated.** `DUPLICATE_READ_ONLY_TOOL_REUSE` is `ACTIVE_SAFE` this phase — it composes the ALREADY ACTIVE, already-certified FG-1C `DuplicateActionSupervisor` (`duplicate-suppression.ts`), so FG-9 introduces no new suppression logic and no new execution-path risk; it only wraps already-happening behavior into a `ForgeGreenOptimizationDecision`/`ForgeGreenOptimizationReceipt` provenance record. `DUPLICATE_CONTEXT_PAGE_TRANSMISSION`, `OPTIONAL_PREFETCH_SUPPRESSION`, and `VERIFICATION_EVIDENCE_REUSE` remain `SHADOW` this phase — implemented and tested as pure candidate detectors against deterministic/adversarial fixtures, not yet wired into live context assembly or ForgeVerify call sites.

**Decision/receipt schema.** `ForgeGreenOptimizationDecision` records identity, candidate evidence, an `expectedEffect` (only fields evidence supports — never guessed), `confidence`, `safetyGuards` (redundancy rationale, invariant, verification proof), and `status`. `ForgeGreenOptimizationReceipt` links before → decision → after → verification with a `resourceDelta` tagged `measured`/`simulated`/`projected` (a `measured` delta requires both a before and after `SustainabilityReceipt` reference, enforced at construction) and a `qualityResult`/`survivedValidation`/`rollbackStatus`. Both are content-addressed, matching the FG-8R fix to `SustainabilityReceipt.receiptId`: an exact retry never duplicates, a genuinely different measurement never silently merges.

**Verification relationship.** FG-9 never invents a parallel verification-cache validity policy — Candidate D only ever proposes reuse for evidence whose validity ForgeVerify itself already explicitly confirmed (`forgeVerifyConfirmedValid: true`, supplied by the caller, never computed by FG-9). Removing verification is never representable as an optimization.

**Mutating tools stay uncacheable.** `write_file`/`edit_file`/`run_command` (installs/deployments run via `run_command`) are disjoint from FG-1C's read-only-suppressible set by construction — proven directly against the real `DuplicateActionSupervisor`, not a reimplementation, in `packages/server/test/fg9-unsafe-mutating.test.ts`.

**Persistence, privacy, fail-open.** Two new append-only `WorkItem` kinds (`forgegreen_optimization_decision`, `forgegreen_optimization_receipt`) through the same `ISessionPersistence` abstraction as every other ForgeGreen receipt. Only ids/hashes/counts/byte-sizes are ever recorded. An FG-9 failure is isolated in its own try/catch, logged, and never affects the agent run or the FG-8/FG-8R measurement it sits beside.

...
export function describeHeaderActivity(status: ActiveWorkCounts): string | null {
  ...
  // Repository indexing is useful background maintenance, but it is not an agent task. Calling
  // it one makes a finished task look as if CodeForge is still executing it.
  if (status.backgroundTasks > 0) parts.push(`${status.backgroundTasks} background task${status.backgroundTasks === 1 ? "" : "s"}`);
 ...
 * standalone chat turns and count on their own; hosted continuations count as tasks, while
 * repository indexing remains visibly separate background work.
 */
export function countRunningTasks(status: ActiveWorkCounts): number {
  ...
  return ownedByTurns + orphanUnits + status.hostedContinuations;
}
